...
(function(window, undefined)
{
	...
	var RuntimeUtils =
	{
		...
		doesMethodBelongToClass : function(method, classObject)
		{
			...
			for(var name in classObject)
			{
				if(classObject[name] === method)
				{
					return true;
				}
			}
			...
			for(name in classObject.prototype)
			{
				if(classObject.prototype[name] === method)
				{
					return true;
				}
			}
	...
	var PObject = function()
	{
		...
		this.dispatch = function(eventName, data)
		{
			...
			{
				var i;
				for(i in this._handlers[eventName])
				{
					if(typeof(this._handlers[eventName][i]) === 'function')
					{
						this._handlers[eventName][i](eventObject);
	...
	var InternalClassUtils =
	{
		...
		{
			var info = {};
			for(var i in CLASS_MODEL.declarationFields)
			{
		...
		parseDefinition : function(definition)
		{
			...
			for(var i in CLASS_MODEL.definitionFields.scopeClass)
			{
			...
			for(i in CLASS_MODEL.definitionFields.scopeInstance)
			{
		...
		transcribeScope : function(from, to, type, scope, ignore)
		{
			...
			for(var name in from)
			{
				if(typeof(ignore) === 'undefined' || ignore.indexOf(name) === -1)
				{
					...
					{
						to[name] = scopingMethod(name, from[name], type, scope, to);
					} else
					{
						...
						to[scopeTargetHolder][name] = scopingMethod(name, from[name], type, scope);
					}

					scopeInfoHolder[name] = scope;
				}
			}
		},
		...
		getClassByName : function(className, imports)
		{
			...
			{
				if(InternalClassUtils.isArray(window[className]) && typeof(imports) !== 'undefined')
				{
				...
				} else if(typeof(window[className]) !== 'undefined' && '_classInfo' in window[className])
				{
		...
		{
			if(typeof(loads) === 'undefined' || loads.length === 0)
			{
				handler();
				return;
			}
			...
			var wrappedHandler = function()
			{
				...
				{
					if(typeof(loads[loads.length - loadsLeft]) !== 'undefined')
					{
		...
		applyInheritance : function(extendedClass, baseClassName, packageImports)
		{
			...
			for(var name in baseClass.prototype)
			{
				if(CLASS_MODEL.extensionDisabled.indexOf(name) === -1)
				{
					...
					if(typeof(extendedClass.prototype[name]) === 'undefined')
					{
		...
		registerClass : function(packageName, className, classObject)
		{
			...
			var completeHandler = function()
			{
				...
				{
					for(var i in InternalClassUtils.classReadyListeners[classObject._classInfo.qualifiedName])
					{
	...
	var Class = function(declaration, definition)
	{
		...
		if(typeof(declarationInfo) === 'undefined' || declarationInfo === null)
		{
	...
	function init()
	{
		...
		for(var i = 0; i < scripts.length; i ++)
		{
			if(scripts[i].hasAttribute('main'))
			{
				...
				{
					Poof.debug = scripts[i].getAttribute('debug');
				}
